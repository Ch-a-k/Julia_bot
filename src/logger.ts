...
export function initLogger(telegram: Telegram): void {
...
function addLogEntry(level: string, message: string): void {
  const entry = {
    timestamp: Date.now(),
    level,
...
async function broadcastToSubscribers(message: string): Promise<void> {
  if (!telegramInstance || logSubscribers.size === 0) return;
...
export function getRecentErrors(count: number = 20): string {
...
// Подписка на логи
export function subscribeToLogs(userId: number): boolean {
  logSubscribers.add(userId);
  return true;
}

// Отписка от логов
export function unsubscribeFromLogs(userId: number): boolean {
...
export function getLogStats(): { total: number; errors: number; warnings: number; subscribers: number } {
  return {
